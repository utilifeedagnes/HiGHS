...
void highs_passModelPointers(
  ...
  HighsStatus status = h->passModel(
      num_col, num_row, num_nz, q_num_nz, a_format, q_format, sense, offset,
      col_cost_ptr, col_lower_ptr, col_upper_ptr, row_lower_ptr, row_upper_ptr,
      a_start_ptr, a_index_ptr, a_value_ptr, q_start_ptr, q_index_ptr,
...
void highs_passLpPointers(
  ...
  HighsStatus status = h->passModel(
      num_col, num_row, num_nz, a_format, sense, offset, col_cost_ptr,
      col_lower_ptr, col_upper_ptr, row_lower_ptr, row_upper_ptr, a_start_ptr,
      a_index_ptr, a_value_ptr, integrality_ptr);
  if (status != HighsStatus::kOk)
    throw py::value_error("Error when passing model");
}
...
void highs_passHessianPointers(Highs* h, const int dim, const int num_nz,
                               const int format, const py::array_t<int> q_start,
  ...
  HighsStatus status = h->passHessian(dim, num_nz, format, q_start_ptr,
                                      q_index_ptr, q_value_ptr);
  if (status != HighsStatus::kOk)
    throw py::value_error("Error when passing Hessian");
}

void highs_writeSolution(Highs* h, const std::string filename,
                         const int style) {
...
void highs_addRow(Highs* h, double lower, double upper, int num_new_nz,
  ...
  HighsStatus status =
      h->addRow(lower, upper, num_new_nz, indices_ptr, values_ptr);

  if (status != HighsStatus::kOk)
    throw py::value_error("Error when adding row");
}

void highs_addRows(Highs* h, int num_cons, py::array_t<double> lower,
  ...
  HighsStatus status = h->addRows(num_cons, lower_ptr, upper_ptr, num_new_nz,
...
void highs_addCol(Highs* h, double cost, double lower, double upper,
  ...
  HighsStatus status =
      h->addCol(cost, lower, upper, num_new_nz, indices_ptr, values_ptr);
...
void highs_addVars(Highs* h, int num_vars, py::array_t<double> lower,
...
void highs_changeColsCost(Highs* h, int num_set_entries,
                          py::array_t<int> indices, py::array_t<double> cost) {
  py::buffer_info indices_info = indices.request();
  ...
  HighsStatus status =
      h->changeColsCost(num_set_entries, indices_ptr, cost_ptr);

  if (status != HighsStatus::kOk)
    throw py::value_error("Error when changing objective coefficients");
}

void highs_changeColsBounds(Highs* h, int num_set_entries,
                            py::array_t<int> indices, py::array_t<double> lower,
                            py::array_t<double> upper) {
  py::buffer_info indices_info = indices.request();
  py::buffer_info lower_info = lower.request();
  py::buffer_info upper_info = upper.request();
  ...
  HighsStatus status =
      h->changeColsBounds(num_set_entries, indices_ptr, lower_ptr, upper_ptr);

  if (status != HighsStatus::kOk)
    throw py::value_error("Error when changing variable bounds");
}

void highs_changeColsIntegrality(Highs* h, int num_set_entries,
  ...
  HighsStatus status =
      h->changeColsIntegrality(num_set_entries, indices_ptr, integrality_ptr);
...
class CallbackTuple {
 public:
  ...
  CallbackTuple(py::object _callback, py::object _cb_data)
      : callback(_callback), callback_data(_cb_data) {}
  ~CallbackTuple() = default;
  py::object callback;
  py::object callback_data;
};

void py_log_callback(HighsLogType log_type, const char* msgbuffer,
                     void* callback_data) {
...
PYBIND11_MODULE(highs_bindings, m) {
  ...
  py::class_<HighsOptions>(m, "HighsOptions")
      ...
      .def_readwrite("simplex_dualise_strategy",
                     &HighsOptions::simplex_dualise_strategy)
  ...
  py::class_<Highs>(m, "_Highs")
      ...
      .def("writeOptions", &Highs::writeOptions, py::arg("filename"),
           py::arg("report_only_deviations") = false)
